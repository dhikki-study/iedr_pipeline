...
BRONZE_INSTALLED_DER = f"{CATALOG}.{BRONZE_SCHEMA}.bronze_installed_der"
STAGING_TABLE = f"{CATALOG}.{SILVER_SCHEMA}.silver_installed_der_stg"
SILVER_TABLE = f"{CATALOG}.{SILVER_SCHEMA}.silver_installed_der"

print("Table Configuration:")
print(f"Bronze:  {BRONZE_INSTALLED_DER}")
...
def extract_and_unpivot_from_bronze(max_timestamp):
    ...
    df_bronze = spark.sql(f"""
        SELECT *
        FROM {BRONZE_INSTALLED_DER}
        WHERE {where_clause}
    """)
    
    bronze_count = df_bronze.count()
    print(f"Extracted {bronze_count:,} records from Bronze")
    
    if bronze_count == 0:
        print("ℹNo new records to process")
    ...
    # STEP 4: Process utility2 (pass through, just select needed columns)
    if utility2_count > 0:
        print("\nStep 3b: Processing utility2 (pass through - already in correct format)...")
        
        # Assuming utility2 already has DER_TYPE and DER_VALUE columns in Bronze
        df_utility2_processed = df_utility2.select(
            ...
            # DER Type (already in long format for utility2)
...
def load_to_staging(df, record_count):
    """
    Load unpivoted records into silver_installed_der_stg using overwrite mode.
    """
    if record_count == 0:
    ...
    print(f"Verified: {staging_count:,} records in {STAGING_TABLE}")
    #"""
    return True
...
def run_bronze_to_staging_installed_der():
    """
    Main orchestration: Bronze → Staging for installed DER
    """
    print("\n" + "="*80)
    print("STARTING BRONZE → STAGING EXTRACTION (INSTALLED DER)")
    print("="*80)
    print(f"⏰ Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
staging_loaded = run_bronze_to_staging_installed_der()